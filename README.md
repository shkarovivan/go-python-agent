...
Запрос может содержать необязательный флаг `local` (по умолчанию `true`),
но пока сервис всегда отвечает локальной моделью — ветвление по флагу в
Python не реализовано. Ответ содержит только поле `result`.
...
| POST  | `/process` | `{"text":"hello"}` (`local` опц., дефолт `true`) | `{"result":"<ответ модели>"}` |
...
curl -X POST http://localhost/process \
  ...
  -d '{"text":"Расскажи короткую шутку"}'
# {"result":"<ответ модели>"}
```
